...
variable "s3_folders" {
  ...
  default     = ["dist/myapp", "versions/myapp"]
}

resource "aws_s3_bucket" "distribution" {
  bucket = "${var.environment}-${var.app_name}-distribution"
...
resource "aws_iam_policy" "s3_distribution_bucket_policy" {
  name = "${var.environment}-${var.app_name}-s3-distribution-bucket"
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:s3:::${var.environment}-${var.app_name}-distribution"
      },
      {
        Action = [
          "s3:GetObject",
