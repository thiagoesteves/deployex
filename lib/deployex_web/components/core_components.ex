defmodule DeployexWeb.CoreComponents do
  @moduledoc """
  ...
  @doc """
  Renders a terminal_modal.

  ## Examples

      <.terminal_modal id="confirm-terminal_modal">
        This is a terminal_modal.
      </.terminal_modal>

  JS commands may be passed to the `:on_cancel` to configure
  the closing/cancel event, for example:

      <.terminal_modal id="confirm" on_cancel={JS.navigate(~p"/posts")}>
        This is another terminal_modal.
      </.terminal_modal>
        ...
        >
          ...
          >
            <td
              ...
              class={["relative p-0", @row_click && "hover:cursor-pointer"]}
            >
              <div class="block px-1 py-1 pr-6 text-xs">
                <span class="absolute -inset-y-px right-0 -left-4 group-hover:bg-zinc-50 sm:rounded-l-xl" />
                <span class={["relative", i == 0 && "font-semibold text-zinc-900"]}>
                  <%= render_slot(col, @row_item.(row)) %>
                </span>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(DeployexWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(DeployexWeb.Gettext, "errors", msg, opts)
    end
  end
