defmodule Deployex.Logs.Server do
  ...
  defp handle_log_update(
         %{
           metadata: %{context: :terminal_index, node: reporter, type: log_key},
           myself: _pid,
           message: message
         },
         %{nodes: nodes, persist_data?: persist_data?, node_logs_tables: node_logs_tables} =
  ...
  defp initialize_log_table(node, current_map) do
    ...
    create_terminal = fn log_type ->
      ...
      {:ok, _pid} =
        Terminal.new(%Terminal{
          ...
          metadata: %{context: :terminal_index, node: node, type: log_type}
        })
    end
    ...
    if instance do
      ...
      Logger.warning("Node #{node} doesn't belong to the same hostname")
    end
