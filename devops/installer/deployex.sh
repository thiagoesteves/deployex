...
version=$(yq '.version' $config_file | sed 's/"/\ /g' )
otp_version=$(yq '.otp_version' $config_file | sed 's/"/\ /g' )
otp_tls_certificates=$(yq '.otp_tls_certificates' $config_file | sed 's/"/\ /g' )
os_target=$(yq '.os_target' $config_file | sed 's/"/\ /g' )
app_name=$(yq '.applications[0].name' $config_file | sed 's/"/\ /g' )
