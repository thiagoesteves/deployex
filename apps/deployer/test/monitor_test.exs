defmodule Deployer.MonitorTest do
  ...
  describe "Running applications" do
    ...
    test "Running application - no pre_commands - elixir" do
      ...
      assert %{status: :running} = Deployer.Monitor.Application.state(instance)
    ...
    test "Check the application doesn't change to running with invalid ref" do
      ...
      assert %{status: :starting} = Deployer.Monitor.Application.state(instance)
    ...
    test "Running pre_commands while application is running - elixir" do
      ...
      {:ok, _pre_commands} =
        Deployer.Monitor.Application.run_pre_commands(instance, pre_commands, :new)
    ...
    test "Restart Application if EXIT message is received" do
      ...
      assert %{status: :running, crash_restart_count: 0} =
               Deployer.Monitor.Application.state(instance)
  ...
  test "Adapter function test" do
    ...
    assert {:ok, _pid} = Deployer.Monitor.start_service(1, make_ref(), [])
    assert :ok = Deployer.Monitor.stop_service(1)
    assert {:ok, %{}} = Deployer.Monitor.state(1)
    assert :ok = Deployer.Monitor.restart(1)
    assert {:ok, []} = Deployer.Monitor.run_pre_commands(1, [], :new)
    assert %{} = Deployer.Monitor.global_name(1)
    assert %{} = Deployer.Monitor.global_name(1, 1)
  end
  ...
  test "Do not change state when an invalid :check_running msg is received" do
    ...
    state = Deployer.Monitor.Application.state(instance)

    send(pid, {:check_running, :any, :any})

    :timer.sleep(100)

    assert state == Deployer.Monitor.Application.state(instance)
